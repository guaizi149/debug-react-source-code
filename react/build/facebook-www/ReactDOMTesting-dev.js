...
function sanitizeURL(url) {
  {
    if (!!isJavaScriptProtocol.test(url)) {
      {
        throw Error( "React has blocked a javascript: URL as a security precaution." );
      }
    }
  }
}
...
if (!(typeof ReactFbErrorUtils.invokeGuardedCallback === 'function')) {
  {
    throw Error( "Expected ReactFbErrorUtils.invokeGuardedCallback to be a function." );
  }
}
...
function createRoot(container, options) {
  ...
  var root = createContainer(container, ConcurrentRoot, hydrate, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride);
...
function registerReactDOMEvent(target, domEventName, isCapturePhaseListener) {
  if (target.nodeType === ELEMENT_NODE) ; else if ( isReactScope(target)) ; else if (isValidEventTarget(target)) {
    ...
    {
      {
        throw Error( "ReactDOM.createEventHandle: setter called on an invalid target. Provide a valid EventTarget or an element managed by React." );
      }
    }
  }
}
...
function createEventHandle(type, options) {
  {
    ...
    if (!allNativeEvents.has(domEventName)) {
      {
        throw Error( "Cannot call unstable_createEventHandle with \"" + domEventName + "\", as it is not an event known to React." );
      }
    }
    ...
    var eventHandle = function (target, callback) {
      if (!(typeof callback === 'function')) {
        {
          throw Error( "ReactDOM.createEventHandle: setter called with an invalid callback. The callback must be a function." );
        }
      }
...
//# sourceMappingURL=ReactDOMTesting-dev.js.map
