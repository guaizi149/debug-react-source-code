...
function shim$1() {
  {
    {
      throw Error( "The current renderer does not support React Scopes. This error is likely caused by a bug in React. Please file an issue." );
    }
  }
...
function requestUpdateLane(fiber) {
  ...
  if (isTransition) {
    if ( warnOnSubscriptionInsideStartTransition && ReactCurrentBatchConfig$2._updatedFibers) {
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  var exitStatus = shouldTimeSlice(root, lanes) && (disableSchedulerTimeoutInWorkLoop || !didTimeout) ? renderRootConcurrent(root, lanes) : renderRootSync(root, lanes);
